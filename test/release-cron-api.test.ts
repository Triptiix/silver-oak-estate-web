...
describe("release and cleanup APIs", () => {
  ...
  it("releases a valid cookie-bound hold and clears the same API-scoped cookie", async () => { const token = signHoldToken({ v: 1, bookingId: "booking", nonce: "nonce", expiresAt: "2030-01-01T00:00:00Z" }, "dummy"); const response = await release(releaseRequest(`soe_booking_hold=${token}`)); expect(response.status).toBe(200); expect(await response.json()).toEqual({ released: true }); const cookie = response.headers.get("set-cookie"); expect(cookie).toContain("Max-Age=0"); expect(cookie).toContain("Path=/api"); expect(cookie).toMatch(/HttpOnly.*SameSite=lax/i); });
  it("rejects an invalid token", async () => expect((await release(releaseRequest("soe_booking_hold=bad"))).status).toBe(400));
  it("rejects an expired token", async () => { const token = signHoldToken({ v: 1, bookingId: "booking", nonce: "nonce", expiresAt: "2020-01-01T00:00:00Z" }, "dummy"); expect((await release(releaseRequest(`soe_booking_hold=${token}`))).status).toBe(400); });
