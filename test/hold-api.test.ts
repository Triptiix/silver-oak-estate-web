...
describe("hold API", () => {
  ...
  it("creates a hold and sets a protected cookie", async () => { const response = await POST(request(input)); expect(response.status).toBe(201); expect(response.headers.get("set-cookie")).toMatch(/soe_booking_hold=.*HttpOnly.*SameSite=lax/i); const body = await response.json(); expect(body.bookingId).toBeUndefined(); expect(body.holdTokenNonce).toBeUndefined(); });
