...
select
  ...
  property.timezone,
  current_date_context.property_local_current_date,
  future_date_context.future_month_start,
  to_char(future_date_context.future_month_start, 'YYYY-MM') as future_month,
  future_date_context.future_month_start as owner_block_date,
  future_date_context.future_month_start + 2 as stale_hold_date,
  future_date_context.future_month_start + 9 as trusted_hold_date,
  future_date_context.future_month_start + 10 as abuse_date
from public.properties as property
cross join lateral (
  select (now() at time zone property.timezone)::date as property_local_current_date
) as current_date_context
cross join lateral (
  select make_date(
    extract(year from current_date_context.property_local_current_date)::integer + 2,
    8,
    1
  ) as future_month_start
) as future_date_context
...
select is(jsonb_array_length(public.get_monthly_availability('silver-oak-estate', (select future_month from phase2_context))->'dates'), 31, 'availability returns every day in the month');
select is((public.get_monthly_availability('silver-oak-estate', (select future_month from phase2_context))->'dates'->0->>'available')::boolean, true, 'empty date is available');

insert into public.inventory_reservations (property_id, reservation_type, status, start_at, end_at, source)
select context.property_id, 'owner_block', 'active', dates.check_in_at, dates.check_out_at, 'phase2-test'
from phase2_context as context
cross join lateral public.resolve_booking_dates(context.property_id, context.owner_block_date) as dates;
select is((public.get_monthly_availability('silver-oak-estate', (select future_month from phase2_context))->'dates'->0->>'available')::boolean, false, 'active owner block is unavailable');
update public.inventory_reservations set status = 'released' where source = 'phase2-test';
select is((public.get_monthly_availability('silver-oak-estate', (select future_month from phase2_context))->'dates'->0->>'available')::boolean, true, 'released reservation is available');
...
select context.property_id, 'temporary_hold', 'active', dates.check_in_at, dates.check_out_at, now() - interval '1 minute', 'phase2-stale', now() - interval '20 minutes'
from phase2_context as context
cross join lateral public.resolve_booking_dates(context.property_id, context.stale_hold_date) as dates;
select is((public.get_monthly_availability('silver-oak-estate', (select future_month from phase2_context))->'dates'->2->>'available')::boolean, true, 'stale active hold is omitted from public availability');
...
select lives_ok($$
  select public.create_booking_hold(
    'silver-oak-estate',(select trusted_hold_date from phase2_context),'Test Guest',null,'+919999000001',null,8,4,null,
...
select throws_ok($$select public.create_booking_hold('silver-oak-estate',(select trusted_hold_date from phase2_context),'Other',null,'+919999000002',null,2,0,null,'31000000-0000-0000-0000-000000000001','32000000-0000-0000-0000-000000000002','different-fingerprint','a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1',10)$$, 'P0001', 'idempotency_conflict', 'mismatched retry is rejected');
select throws_ok($$select public.create_booking_hold('silver-oak-estate',(select abuse_date from phase2_context),'Test Guest',null,'+919999000001',null,2,0,null,'31000000-0000-0000-0000-000000000002','32000000-0000-0000-0000-000000000002','fingerprint-one','a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1',10)$$, 'P0001', 'hold_abuse_limit', 'identity cannot hoard another hold');
select throws_ok($$select public.create_booking_hold('silver-oak-estate',(select trusted_hold_date from phase2_context),'Conflict',null,'+919999000003',null,2,0,null,'31000000-0000-0000-0000-000000000003','32000000-0000-0000-0000-000000000003','fingerprint-three','a3a3a3a3a3a3a3a3a3a3a3a3a3a3a3a3a3a3a3a3a3a3a3a3a3a3a3a3a3a3a3a3',10)$$, 'P0001', 'date_unavailable', 'overlapping hold receives controlled conflict');
